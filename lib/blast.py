...
class BLAST(container):
    ...
    ###################################################
    def __init__(self,program,seqtype="dna",path="",ref="",query="",source_path=""):
        self.path = path
    ...
    def execute(self):
        if not self.success or not self.cline:
            return
        output = self.process(self.cline).decode()
        if not output:
            return False
        oParser = parser(self.program,output)
        self.container = oParser()
        for tmpfname in ("tmp.fas","seq1.fas","seq2.fas","ref.fas",
                            "tmp.nhr","tmp.nin","tmp.nsq","tmp.phr","tmp.pin","tmp.psq"):
            fname = os.path.join(self.path,tmpfname)
    ...
    def _set_cline(self,blastdb=None):
        ...
        if sys.platform == "win32":
            if blastdb != None:
                self.cline = " ".join([os.path.join(self.path,"formatdb"),"-i "+self.sbjct,"-p "+flg_prot,"-n "+blastdb])
            elif self.program == "bl2seq":
                default = "-G 5 -E 2 -q -2 -r 1 -e 1.0"
                self.cline = " ".join([os.path.join(self.path,"bl2seq"),"-p "+algorithm,"-i "+self.query,"-j "+self.sbjct,
                default])
            elif self.program == "blast":
                default = "-G 5 -E 2 -q -2 -r 1 -e 1.0"
                self.cline = " ".join([os.path.join(self.path,"blastall"),"-p "+algorithm,"-i "+self.query,
                    "-d "+"\""+os.path.join(self.path,self.sbjct)+"\"",default])
        elif sys.platform in ("linux2"):
            if blastdb != None:
                self.cline = " ".join([os.path.join(self.path,"makeblastdb"),"-in "+self.sbjct,
                    "-dbtype "+dbtype,"-out "+blastdb])
            elif self.program == "blast":
                default = "-gapopen 5 -gapextend 2 -evalue 1.0"
                self.cline = " ".join([os.path.join(self.path,algorithm),"-query "+self.query,
                    default,"-db "+os.path.join(self.path,self.sbjct)])
            elif self.program == "bl2seq":
                default = "-gapopen 5 -gapextend 2 -evalue 1.0"
                self.cline = " ".join([os.path.join(self.path,algorithm),"-query "+self.query,"-subject "+self.sbjct,
                    default])

    def _validate(self,source_path=""):
        if self.program in ("bl2seq","bl2seqp"):
            # 1) query is a StringIO object	TODO
            ...
            if self.query and not os.path.exists(self.query):
                ...
                try:
                    ...
                    seqname = ">Seq1"
                    sequence = self.query
                self.IO.save("%s\n%s" % (seqname,sequence),os.path.join(self.path,"seq1.fas"))
                self.query = os.path.join(self.path,"seq1.fas")
            elif self.query and os.path.exists(self.query) and self.query[self.query.rfind("."):].upper() in (".GBK",".GB"):
                self.query = self.IO.save(self.IO.openGBK(self.query,"FASTA"),os.path.join(self.path,"seq1.fas"))
                self.query_genemap = self.IO.getGeneMap()
            if not os.path.exists(self.query):
                self.success = False
                return
            
            # 1) sbjct is a StringIO object	TODO
            ...
            if self.sbjct and not os.path.exists(self.sbjct):
                ...
                try:
                    ...
                    seqname = ">Seq2"
                    sequence = self.sbjct
                self.IO.save("%s\n%s" % (seqname,sequence),os.path.join(self.path,"seq2.fas"))
                self.sbjct = os.path.join(self.path,"seq2.fas")
            elif self.sbjct and os.path.exists(self.sbjct) and self.sbjct[self.sbjct.rfind("."):].upper() in (".GBK",".GB"):
                self.sbjct = self.IO.save(self.IO.openGBK(self.sbjct,"FASTA"),os.path.join(self.path,"seq2.fas"))
            ...
            '''
            if self.query and not os.path.exists(self.query):
                self.IO.save(">locus\n"+self.query,os.path.join(self.path,"tmp.fas"))
                self.query = os.path.join(self.path,"tmp.fas")
            elif self.query and self.query[self.query.rfind("."):].upper() in (".GBK",".GB"):
                outpath = os.path.join(self.path,"tmp.fas")
            ...
            if self.sbjct and self.sbjct[self.sbjct.rfind("."):].upper() in (".GBK",".GB"):
                outpath = os.path.join(self.path,"ref.fas")
            ...
            elif self.sbjct and self.sbjct[self.sbjct.rfind("."):].upper() in (".FASTA",".FST",".FAS",".FNA",".FSA"):
                outpath = os.path.join(self.path,"ref.fas")
                self.sbjct = os.path.join(source_path,os.path.basename(self.sbjct))
    ...
    def fasta2blast(self):
        if not os.path.exists(self.sbjct):
            return
        blastdb = os.path.join(self.path,"tmp")
...
class sys_parser:
    ...
    def _parse(self):
        ...
        #### TEMP
        IO = seq_io.IO()
        IO.save(self.raw_text,"wintmp.out")
        '''
        if self.program in ("bl2seq","bl2seqp"):
